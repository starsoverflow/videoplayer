...
HRESULT EVRCustomPresenter::InitServicePointers(IMFTopologyServiceLookup *pLookup)
{
  ...
  JIF(pLookup->LookupService(
    MF_SERVICE_LOOKUP_GLOBAL, 0,
    MR_VIDEO_MIXER_SERVICE, IID_PPV_ARGS(&m_pMixer), &dwObjectCount
    ));

  // Make sure that we can work with this mixer.
  JIF(ConfigureMixer(m_pMixer));
...
// GetNativeVideoSize
//
// Gets the current videoSize and VideoAspectRatio
//-----------------------------------------------------------------------------
HRESULT EVRCustomPresenter::GetNativeVideoSize(SIZE* pszVideo, SIZE* pszARVideo) 
{ 
  HRESULT hr =  S_OK; 

  // modified by star: needn't check this since m_szVideo is already written when program connects the pins
  // JIF(CheckShutdown());
...
HRESULT EVRCustomPresenter::SetVideoPosition(const MFVideoNormalizedRect* pnrcSource, const LPRECT prcDest)
{
  ...
  if (pnrcSource)
  {
    ...
    if (m_pMixer)
    {
      JIF(SetMixerSourceRect(m_pMixer, m_nrcSource));
    }
  }
...
{
	if (dwAspectRatioMode == Stretch)
	{
		m_pD3DPresentEngine->ShowBorder(false);
		return S_OK;
	}
	else if (dwAspectRatioMode == NoStretch)
	{
		m_pD3DPresentEngine->ShowBorder(true);
		return S_OK;
	}
	else
		return E_INVALIDARG;
...
{
	if (pdwAspectRatioMode) *pdwAspectRatioMode = m_pD3DPresentEngine->IsShowBorder();
	return S_OK;
}
...
HRESULT EVRCustomPresenter::SetBorderColor(COLORREF Clr)
{
	m_pD3DPresentEngine->SetBorderColor(Clr);
	return S_OK;
}

HRESULT EVRCustomPresenter::GetBorderColor(COLORREF * pClr)
{
	m_pD3DPresentEngine->GetBorderColor(pClr);
	return S_OK;
}
...
HRESULT EVRCustomPresenter::ConfigureMixer(IMFTransform *pMixer)
{
  ...
  JIF(pMixer->QueryInterface(IID_PPV_ARGS(&pDeviceID)));

  JIF(pDeviceID->GetDeviceID(&mixerID));
...
HRESULT EVRCustomPresenter::StartFrameStep()
{
  ...
  if (m_FrameStep.state == FRAMESTEP_WAITING_START)
  {
    ...
    {
      JIF(m_FrameStep.samples.RemoveFront(&pSample));

      JIF(DeliverFrameStepSample(pSample));
  ...
  else if (m_FrameStep.state == FRAMESTEP_NONE)
  {
    ...
    while (!m_FrameStep.samples.IsEmpty())
    {
      ...
      JIF(DeliverSample(pSample, FALSE));
...
HRESULT EVRCustomPresenter::CreateOptimalVideoType(IMFMediaType* pProposedType, IMFMediaType **ppOptimalType)
{
  ...
  JIF(MFCreateMediaType(&pMTOptimal));
  JIF(pProposedType->CopyAllItems(pMTOptimal));
  ...
  JIF(MFSetAttributeRatio(pMTOptimal, MF_MT_PIXEL_ASPECT_RATIO, 1, 1));

  // Calculate the output rectangle based on the media type.
  JIF(CalculateOutputRectangle(pProposedType, &rcOutput));

  // Set the extended color information: Use BT.709
  JIF(pMTOptimal->SetUINT32(MF_MT_TRANSFER_FUNCTION, MFVideoTransFunc_709));
  JIF(pMTOptimal->SetUINT32(MF_MT_VIDEO_PRIMARIES, MFVideoPrimaries_BT709));
  JIF(pMTOptimal->SetUINT32(MF_MT_VIDEO_NOMINAL_RANGE, MFNominalRange_0_255));
  JIF(pMTOptimal->SetUINT32(MF_MT_VIDEO_LIGHTING, MFVideoLighting_office));

  // Set the target rect dimensions.
  JIF(MFSetAttributeSize(pMTOptimal, MF_MT_FRAME_SIZE, rcOutput.right, rcOutput.bottom));

  // Set the geometric aperture, and disable pan/scan.
  displayArea = MakeArea(0, 0, rcOutput.right, rcOutput.bottom);
  JIF(pMTOptimal->SetUINT32(MF_MT_PAN_SCAN_ENABLED, FALSE));
  JIF(pMTOptimal->SetBlob(MF_MT_GEOMETRIC_APERTURE, (UINT8*)&displayArea, sizeof(displayArea)));

  // Set the pan/scan aperture and the minimum display aperture. We don't care
  // about them per se, but the mixer will reject the type if these exceed the
  // frame dimentions.
  JIF(pMTOptimal->SetBlob(MF_MT_PAN_SCAN_APERTURE, (UINT8*)&displayArea, sizeof(displayArea)));
  JIF(pMTOptimal->SetBlob(MF_MT_MINIMUM_DISPLAY_APERTURE, (UINT8*)&displayArea, sizeof(displayArea)));
...
HRESULT EVRCustomPresenter::CalculateOutputRectangle(IMFMediaType *pProposedType, RECT *prcOutput)
{
  ...
  JIF(MFGetAttributeSize(pProposedType, MF_MT_FRAME_SIZE, &srcWidth, &srcHeight));

  // Get the source's display area.
  JIF(GetVideoDisplayArea(pProposedType, &displayArea));
...
HRESULT EVRCustomPresenter::SetMediaType(IMFMediaType *pMediaType)
{
  ...
  // Set VideoSize and AR
  JIF(pMediaType->GetUINT64(MF_MT_FRAME_SIZE, (UINT64*)&u64));
  m_szVideo.cx = u64.HighPart;
  m_szVideo.cy = u64.LowPart;

  JIF(GetAspectRatio(pMediaType, &m_szARVideo.cx, &m_szARVideo.cy));
  ...
  JIF(m_pD3DPresentEngine->CreateVideoSamples(pMediaType, sampleQueue));
  ...
  {
    JIF(sampleQueue.GetItemPos(pos, &pSample));
    JIF(pSample->SetUINT32(MFSamplePresenter_SampleCounter, m_TokenCounter));
    SafeRelease(&pSample);
  }

  // Add the samples to the sample pool.
  JIF(m_SamplePool.Initialize(sampleQueue));
...
HRESULT EVRCustomPresenter::IsMediaTypeSupported(IMFMediaType *pProposed)
{
  ...
  JIF(pProposed->GetRepresentation(FORMAT_VideoInfo2, (void**)&pAMMedia));
  ...
  JIF(pProposed->IsCompressedFormat(&bCompressed));
  JIF(pProposed->GetUINT32(MF_MT_INTERLACE_MODE, (UINT32*)&InterlaceMode));
  ...
  // Validate the format.
  JIF(GetFourCC(pProposed, (DWORD*)&d3dFormat));

  // The D3DPresentEngine checks whether the format can be used as
  // the back-buffer format for the swap chains.
  JIF(m_pD3DPresentEngine->CheckFormat(d3dFormat));


  JIF(MFGetAttributeSize(pProposed, MF_MT_FRAME_SIZE, &width, &height));
...
HRESULT EVRCustomPresenter::ProcessOutput()
{
  ...
  else
  {
    ...
    JIF(TrackSample(pSample));

    // Schedule the sample.
    if ((m_FrameStep.state == FRAMESTEP_NONE) || bRepaint)
    {
      JIF(DeliverSample(pSample, bRepaint));
    }
    else
    {
...
HRESULT EVRCustomPresenter::DeliverFrameStepSample(IMFSample *pSample)
{
  ...
  // Modified by star 2017.1.29  delete IsScrubbing()
...
HRESULT EVRCustomPresenter::OnSampleFree(IMFAsyncResult *pResult)
{
  ...
  JIF(pResult->GetObject(&pObject));
  JIF(pObject->QueryInterface(__uuidof(IMFSample), (void**)&pSample));
  ...
  if (m_FrameStep.state == FRAMESTEP_SCHEDULED)
  {
    ...
    JIF(pSample->QueryInterface(__uuidof(IMFSample), (void**)&pUnk));

    if (m_FrameStep.pSampleNoRef == (DWORD_PTR)pUnk)
    {
      // Notify the EVR.
      JIF(CompleteFrameStep(pSample));
    }
...
LONGLONG GetMediaTypeMerit(IMFMediaType *pMediaType)
{
  ...
  LONGLONG Merit = 0;
  HRESULT hr = S_OK;
  JIF(pMediaType->GetRepresentation(FORMAT_MFVideoFormat, (void**)&pAMMedia));
...
HRESULT ClearDesiredSampleTime(IMFSample *pSample)
{
  ...
  if (SUCCEEDED(hr))
  {
    ...
    JIF(pSample->SetUINT32(MFSamplePresenter_SampleCounter, counter));

    if (pUnkSwapChain)
      JIF(pSample->SetUnknown(MFSamplePresenter_SampleSwapChain, pUnkSwapChain));
  }
