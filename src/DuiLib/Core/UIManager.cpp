...
//#endif
    ...
    case WM_CLOSE:
        ...
        break;
		// process WM_KILLFOCUS added by star
	case WM_KILLFOCUS:
	{
		POINT pt = { 0 };
		GetCursorPos(&pt);
   ...
   case WM_PAINT:
	   {
		   ...
		   if(m_bAlphaBackground)
		   {
			   ...
			   if (!m_bIsRestore)
			   {
				   // added by star  win7平台下 GetUpdateRect只能获取屏幕内的部分，是这个导致了窗体移到屏幕外后改变大小，屏幕外的部分不重绘
				   // 经过测试也不怎么影响效率
				   rcPaint = rcClient;
	   ...
	   // added by star 下面这段代码使得窗口大小变化后能够改变HOTIMAGE、ToolTip等信息  在需要时设置为true
	   if (!m_bUpdateNeeded && m_bResize)
	   {
		   m_bResize = false;
		   POINT rs_point;
		   GetCursorPos(&rs_point);
		   ScreenToClient(m_hWndPaint, &rs_point);
		   CControlUI* pNewHover = FindControl(rs_point);
		   if (pNewHover != NULL && pNewHover->GetManager() != this) return true;
		   TEventUI event = { 0 };
		   event.ptMouse = rs_point;
		   event.dwTimestamp = ::GetTickCount();
...
#define WM_MOUSEHWHEEL 0x020E
	// Add support for horizontal scroll wheel by star
	case WM_MOUSEHWHEEL:
	{ 
		POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
		::ScreenToClient(m_hWndPaint, &pt);
		...
		int zDelta = (int)(short)HIWORD(wParam);
		TEventUI event = { 0 };
		event.Type = UIEVENT_SCROLLHWHEEL;
		event.pSender = pControl;
		event.wParam = MAKELPARAM(zDelta < 0 ? SB_LINELEFT : SB_LINERIGHT, 0);
		event.lParam = lParam;
		event.wKeyState = MapKeyState();
		event.dwTimestamp = ::GetTickCount();
		pControl->Event(event);
...
void CPaintManagerUI::SetFocus(CControlUI* pControl)
{
	...
	//无焦点窗口不做任何处理 skilla
	if (!m_bUnfocusPaintWindow) if (hFocusWnd != m_hWndPaint && pControl != m_pFocus) ::SetFocus(m_hWndPaint);
    // Already has focus?
