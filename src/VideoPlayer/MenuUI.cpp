...
namespace DuiLib
{
	...
	void CMenuElementUI::ShowSubMenu()
	{
		...
		m_pExtendMenu->EstimateSize({ 0,0 });
		...
		// modified by star
		if (p.y + m_pExtendMenu->GetFixedHeight()>rcArea.bottom)
		{
			p.y = p.y - m_pExtendMenu->GetFixedHeight() + this->GetHeight() + this->GetPadding().bottom + this->GetPadding().top;
			if (p.y<rcArea.top)
			{
				p.y = rcArea.bottom - m_pExtendMenu->GetFixedHeight();
			}

		}
		else
		{
			p.y = p.y - this->GetPadding().bottom - this->GetPadding().top;
		}
	...
	void CMenuElementUI::SetEnabled(bool bEnabled)
	{
		return CContainerUI::SetEnabled(bEnabled);
	}
	...
	{
		// modified by star 通过高亮的item来判断
		/*
		CMenuElementUI* pMenuItem = static_cast<CMenuElementUI*>(pControl->GetInterface(_T("MenuItem")));
		*/
		CMenuElementUI* pMenuItem=GetCurItem();
		if (pMenuItem)
		{
			SKILL_ASSERT(m_pOwnerTree);
	...
	SIZE CMenuUI::EstimateSize(SIZE szAvailable)
	{
		...
		for (int it = 0; it < GetCount(); it++) {
			...
			cXY.cy += pControl->GetPadding().top;
			cXY.cy += pControl->GetPadding().bottom;
			cXY.cy += sz.cy;
			if (cXY.cx < sz.cx)
				cXY.cx = sz.cx;
		}
	...
	void CDuiMenu::LoadFromXml(LPCTSTR xmlName)
	{
		...
		m_menus = static_cast<CMenuContainer*>(builder.Create(xmlName,NULL,this));
		...
		for (int i=0;i<count;i++)
		{
			...
			// bug fixed by star NULL changed to m_hWndOwner
			pMenu->InitMenu(m_hWndOwner);
			pMenu->SetOwnerTree(this);
			if(pMenu->IsMajor()) 
			{
				SKILL_ASSERT(!m_pMajorMenu);
				m_pMajorMenu = pMenu;
			}
	...
	void CDuiMenu::LoadPopupMenu(HWND hwndParent)
	{
	...
	LPCTSTR CDuiMenu::TrackPopupMenu(int x,int y)
	{
		m_pMajorMenu->EstimateSize({ 0,0 });
	...
	int CDuiMenu::RunMenu()
	{
		...
		while(TRUE)
		{
			...
			if(PeekMessage(&msg, NULL, 0, 0, PM_REMOVE))
			{
				...
				// 没必要拦截，会影响程序逻辑 modified by star
				/*
				//拦截非菜单窗口的MouseMove消息
